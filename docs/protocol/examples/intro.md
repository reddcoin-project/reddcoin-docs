...
- `reddcoin-qt` provides a combination full Reddcoin peer and wallet frontend. From the Help menu, you can access a console where you can enter the [RPC](../reference/rpc/index.html) commands used throughout this document.
- `reddcoind` is more useful for programming: it provides a full peer which you can interact with through [RPCs](../reference/rpc/index.html) to port 45445 (or 55445 for testnet).
- `reddcoin-cli` allows you to send [RPC](../reference/rpc/index.html) commands to `reddcoind` from the command line. For example, `reddcoin-cli help`
...
To use `reddcoind` and `reddcoin-cli`, you will need to add a [RPC](../reference/rpc/index.html) password to your `reddcoin.conf` file. Both programs will read from the same file if both run on the same system as the same user, so any long random password will work:
...
For development, it’s safer and cheaper to use Reddcoin’s test [network](../devguide/p2p_network.html) (testnet) or regression test mode (regtest) described below.
