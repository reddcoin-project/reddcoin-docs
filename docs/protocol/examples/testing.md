...
Many developers consider regtest mode the preferred way to develop new applications. The following example will let you create a regtest environment after you first [configure reddcoind](../examples/index).
...
You can now use Reddcoin Core [RPCs](../reference/rpc/index) prefixed with `reddcoin-cli -regtest`.

Regtest wallets and block chain state (chainstate) are saved in the `regtest` subdirectory of the Reddcoin Core configuration directory. You can safely delete the `regtest` subdirectory and restart Reddcoin Core to start a new regtest. (See the [Developer Examples Introduction](../examples/index) for default configuration directory locations on various operating systems. Always back up mainnet wallets before performing dangerous operations such as deleting.)
