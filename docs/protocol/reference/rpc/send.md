...
[
  {                                 (json object)
    "address": amount,              (numeric or string, required) A key-value pair. The key (string) is the reddcoin address, the value (float or string) is the amount in RDD
    ...
  },
  {                                 (json object)
...
Specify a fee rate in reddoshi/vB.
...
```
{
  ...
  "change_address": "hex",          (string, optional, default=pool address) The reddcoin address to receive the change
  ...
  "estimate_mode": "str",           (string, optional, default="unset") The fee estimate mode, must be one of (case insensitive):
                                    ...
                                    "economical"
                                    "conservative"
  "fee_rate": amount,               (numeric or string, optional, default=not set, fall back to wallet fee estimation) Specify a fee rate in reddoshi/vB.
...
Send 0.2 RDD with a fee rate of 1 reddoshi/vB using the options argument:

```
reddcoin-cli send '{"bc1q09vm5lfy0j5reeulh4x5752q25uqqvz34hufdl": 0.2}' null "unset" null '{"fee_rate": 1}'
```

Send 0.3 RDD with a fee rate of 25 reddoshi/vB using named arguments:

```
reddcoin-cli -named send outputs='{"bc1q09vm5lfy0j5reeulh4x5752q25uqqvz34hufdl": 0.3}' fee_rate=25
```

Create a transaction that should confirm the next block, with a specific input, and return result without adding to wallet or broadcasting to the network:

```
reddcoin-cli send '{"bc1q09vm5lfy0j5reeulh4x5752q25uqqvz34hufdl": 0.1}' 1 economical '{"add_to_wallet": false, "inputs": [{"txid":"a08e6907dbbd3d809776dbfc5d82e371b764ed838b5655e72f463568df1aadf0", "vout":1}]}'
```
