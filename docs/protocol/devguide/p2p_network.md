...
Connecting to a peer is done by sending a [“version” message](../reference/p2p_networking#version), which contains your version number, block, and current time to the remote node. The remote node responds with its own [“version” message](../reference/p2p_networking#version). Then both nodes send a [“verack” message](../reference/p2p_networking#verack) to the other node to indicate the connection has been established.

Once connected, the client can send to the remote node `getaddr` and [“addr” messages](../reference/p2p_networking#addr) to gather additional peers.
...
The block inventories appear in the [“inv” message](../reference/p2p_networking#inv) in the same order they appear in the block chain, so this first [“inv” message](../reference/p2p_networking#inv) contains inventories for blocks 1 through 501. (For example, the hash of block 1 is 4860…0000 as seen in the illustration above.)
...
When the IBD node receives the second [“inv” message](../reference/p2p_networking#inv), it will request those blocks using [“getdata” messages](../reference/p2p_networking#getdata). The sync node will respond with [“block” messages](../reference/p2p_networking#block). Then the IBD node will request more inventories with another [“getblocks” message](../reference/p2p_networking#getblocks)—and the cycle will repeat until the IBD node is synced to the tip of the block chain. At that point, the node will accept blocks sent through the regular block broadcasting described in a later subsection.
...
| --------------------------------------------------------- | -------- | ------------------------------------------------ |
| [“getblocks”](../reference/p2p_networking#getblocks) | IBD→Sync | One or more header hashes                        |
| [“inv”](../reference/p2p_networking#inv)             | Sync→IBD | Up to 500 block inventories (unique identifiers) |
| [“getdata”](../reference/p2p_networking#getdata)     | IBD→Sync | One or more block inventories                    |
| [“block”](../reference/p2p_networking#block)         | Sync→IBD | One serialized block                             |

### Headers-First

[Reddcoin Core 0.10.0](https://github.com/reddcoin-project/reddcoin/releases) uses an initial block download (IBD) method called *headers-first*. The goal is to download the headers for the best header chain, partially validate them as best as possible, and then download the corresponding blocks in parallel. This solves several problems with the older blocks-first IBD method.
...
| Message                                                     | From→To    | Payload                                                  |
| ----------------------------------------------------------- | ---------- | -------------------------------------------------------- |
| [“getheaders”](../reference/p2p_networking#getheaders) | IBD→Sync   | One or more header hashes                                |
| [“headers”](../reference/p2p_networking#headers)       | Sync→IBD   | Up to 2,000 block headers                                |
| [“getdata”](../reference/p2p_networking#getdata)       | IBD→*Many* | One or more block inventories derived from header hashes |
| [“block”](../reference/p2p_networking#block)           | *Many*→IBD | One serialized block                                     |
...
- [Standard Block Relay](/glossary/terms#term-standard-block-relay)**:** the miner, acting as a standard relay node, sends an [“inv” message](../reference/p2p_networking#inv) to each of its peers (both full node and SPV) with an inventory referring to the new block. The most common responses are:
  ...
  - Each Simplified Payment Verification (SPV) client that wants the block replies with a [“getdata” message](../reference/p2p_networking#getdata) typically requesting a merkle block.

  The miner replies to each request accordingly by sending the block in a [“block” message](../reference/p2p_networking#block), one or more headers in a [“headers” message](../reference/p2p_networking#headers), or the merkle block and transactions relative to the SPV client’s bloom filter in a [“merkleblock” message](../reference/p2p_networking#merkleblock) followed by zero or more [“tx” messages](../reference/p2p_networking#tx).

- Direct Headers Announcement**:** a relay node may skip the round trip overhead of an [“inv” message](../reference/p2p_networking#inv) followed by `getheaders` by instead immediately sending a [“headers” message](../reference/p2p_networking#headers) containing the full header of the new block. A HF peer receiving this message will partially validate the block header as it would during headers-first IBD, then request the full block contents with a [“getdata” message](../reference/p2p_networking#getdata) if the header is valid. The relay node then responds to the `getdata` request with the full or filtered block data in a `block` or [“merkleblock” message](../reference/p2p_networking#merkleblock), respectively. A HF node may signal that it prefers to receive `headers` instead of `inv` announcements by sending a special [“sendheaders” message](../reference/p2p_networking#sendheaders) during the connection handshake.
...
| Message                                                       | From→To     | Payload                                                                               |
| ------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------- |
| [“inv”](../reference/p2p_networking#inv)                 | Relay→*Any* | The inventory of the new block                                                        |
| [“getdata”](../reference/p2p_networking#getdata)         | BF→Relay    | The inventory of the new block                                                        |
| [“getheaders”](../reference/p2p_networking#getheaders)   | HF→Relay    | One or more header hashes on the HF node’s best header chain (BHC)                    |
| [“headers”](../reference/p2p_networking#headers)         | Relay→HF    | Up to 2,000 headers connecting HF node’s BHC to relay node’s BHC                      |
| [“block”](../reference/p2p_networking#block)             | Relay→BF/HF | The new block in [serialized format](../reference/block_chain#serialized-blocks) |
| [“merkleblock”](../reference/p2p_networking#merkleblock) | Relay→SPV   | The new block filtered into a merkle block                                            |
| [“tx”](../reference/p2p_networking#tx)                   | Relay→SPV   | Serialized transactions from the new block that match the bloom filter                |
...
In order to send a transaction to a peer, an [“inv” message](../reference/p2p_networking#inv) is sent. If a `getdata` response message is received, the transaction is sent using `tx`. The peer receiving this transaction also forwards the transaction in the same manner, given that it is a valid transaction.
